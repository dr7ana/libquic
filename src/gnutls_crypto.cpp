...
namespace oxen::quic
{
    ...
    /*
     * gnutls debug setup, left here for convenience until confident the gnutls stuff is all working
     * to use, uncomment this block and the corresponding block in the header and line in the constructor below
     *
    bool GNUTLSCreds::gnutls_debug_initialized = false;

    void GNUTLSCreds::init_gnutls_debug()
    {
        if (gnutls_debug_initialized) return;

        gnutls_debug_initialized = true;
